...
final AS (
    ...
    FROM t
    FULL JOIN acc 
      ON t.bar_account::text = acc.bar_account::text
    FULL JOIN cust 
      ON t.bar_customer::text = cust.bar_customer::text
    FULL JOIN ent 
      ON upper(t.bar_entity::text) = upper(ent.bar_entity::text)
    FULL JOIN fnc 
      ON t.bar_function::text = fnc.bar_function::text
    FULL JOIN prd 
      ON t.bar_product::text = prd.bar_product::text
    FULL JOIN cm 
      ON t.erp_customer_sold_to::text = cm.erp_customer_number::text 
     AND t.erp_source::text = cm.erp_source::text
    FULL JOIN mm 
      ON t.erp_material::text = mm.erp_material_number::text 
