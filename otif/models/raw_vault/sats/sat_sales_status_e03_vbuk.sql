{{ config(
    ...
    unique_key = 'SALES_UNIQUE_HK',
    rank_source_models = 'v_stg_e03_sales_status_vbuk',
    cluster_by=["VBELN"]
    )
}}

{%- set source_model = "v_stg_e03_sales_status_vbuk" -%}
{%- set src_pk = "SALES_HK" -%}
...
{%- set src_source = "RECORD_SOURCE" -%}
	...
	{% if target_exists() %}
	  ...
	  --Below was added to take advantage of cluster key
	    AND CAST(sats.LOAD_EFF_DT as DATE)  <= TO_DATE(tgt.LOAD_EFF_DT)
	    )
{% endif %}