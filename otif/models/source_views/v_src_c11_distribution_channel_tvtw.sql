...
FROM {{ source('SAP_C11', 'VW_TVTW') }}
--WHERE CAST(EVENTDTS AS DATE) <=  TO_DATE('2021-11-30')
--UNION ALL
--SELECT 2 AS QRY_NBR, *
--FROM {{ source('SAP_C11', 'VW_TVTW_CHANGES') }}
--WHERE CAST(EVENDTS AS DATE) <= TO_DATE('2021-11-30')
) x
QUALIFY
RANK() OVER (PARTITION BY MANDT,VTWEG,{{ var('src_grain') }} ORDER BY QRY_NBR ASC, HEADER_CHANGE_SEQ DESC) = 1