...
FROM {{ source('SAP_E03', 'VW_MARA') }}
--WHERE CAST(EVENTDTS AS DATE) <=  TO_DATE('2021-11-30')
UNION ALL
SELECT 2 AS QRY_NBR, *
FROM {{ source('SAP_E03', 'VW_MARA_CHANGES') }}
--WHERE CAST(EVENDTS AS DATE) <= TO_DATE('2021-11-30')
) x
QUALIFY
RANK() OVER (PARTITION BY MANDT,MATNR,{{ var('src_grain') }} ORDER BY QRY_NBR ASC, HEADER_CHANGE_SEQ DESC) = 1