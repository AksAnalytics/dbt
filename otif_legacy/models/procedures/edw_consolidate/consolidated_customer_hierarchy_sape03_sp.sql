CREATE OR REPLACE PROCEDURE edw_consolidated.consolidated_customer_hierarchy_sape03_sp()
	LANGUAGE plpgsql
AS $$
...
begin
	...
	drop table if exists edw.edw_stage.consolidated_customer_hierarchy_stg_e03;
	create table edw.edw_stage.consolidated_customer_hierarchy_stg_e03 as
	with  knvh as (
                  select distinct kunnr, hkunnr ,hityp, vkorg , vtweg, spart
                  from edw.sape03.knvh_current  knvh
	...
	, src as (
            ...
            ,  vtweg, spart
            from knvh
            join edw.sape03.kna1_current  as kna1
            on (kna1.kunnr = knvh.kunnr )            
            )
	select 'E03' as source_sys, * from  src;

    RAISE NOTICE 'STAGE TABLE CREATED';

	drop table if exists consolidated_customer_hierarchy_e03;

    create temporary table consolidated_customer_hierarchy_e03_B as
	...
	(
  	...
  	where ktokd in ('0001','0002') and hityp='B'
  	union all
  	...
  	where c.kunnr = p.hkunnr and c.hityp = p.hityp and c.vkorg = p.vkorg and c.vtweg = p.vtweg and c.spart = p.spart  
  		and c.source_sys = p.source_sys and level <=7 and c.hityp='B'
  	)
  	select distinct source_sys,customer as CUSTOMER_CONS ,kunnr  as PARENT_CONS, hityp as HIER_TYP , name1 as PARENT_NAME
    ...
    create temporary table consolidated_customer_hierarchy_e03_D as
	with recursive p(source_sys,customer ,kunnr, hityp, name1,ktokd, vkorg , hkunnr, level, vtweg,spart) as
	(
  	select source_sys,customer ,kunnr, hityp, name1,ktokd, vkorg , hkunnr, 1 as level, vtweg,spart
  	from edw.edw_stage.consolidated_customer_hierarchy_stg_e03 where hityp='D'
  	union all
  	select p.source_sys,p.customer ,p.hkunnr, c.hityp, c.name1,c.ktokd, c.vkorg , c.hkunnr, level+1, c.vtweg, c.spart
  	from edw.edw_stage.consolidated_customer_hierarchy_stg_e03 c,  p
  	where c.kunnr = p.hkunnr and c.hityp = p.hityp and c.vtweg = p.vtweg and c.spart = p.spart  
  		and c.source_sys = p.source_sys and level <=7 and c.hityp='D'
  	)
  	select distinct source_sys,customer as CUSTOMER_CONS ,kunnr  as PARENT_CONS, hityp as HIER_TYP , name1 as PARENT_NAME
  	,ktokd as ACCT_GRP, vkorg as SALESORG_CONS
    ...
    create temporary table consolidated_customer_hierarchy_e03 as
    Select * from consolidated_customer_hierarchy_e03_B
    UNION ALL
    Select * from consolidated_customer_hierarchy_e03_D;
 
    
 
    RAISE NOTICE 'TEMPORARY TABLE CREATED';
 
    delete from edw.edw_consolidated.consolidated_customer_hierarchy where source_sys='E03';
    ...
    select * from consolidated_customer_hierarchy_e03;
