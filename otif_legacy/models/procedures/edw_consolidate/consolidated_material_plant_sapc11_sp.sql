CREATE OR REPLACE PROCEDURE edw_consolidated.consolidated_material_plant_sapc11_sp()
	LANGUAGE plpgsql
AS $$
...
begin
    ...
    drop table if exists CONSOLIDATED_MATERIAL_PLANT_STG_SAPC11;
   
	CREATE TEMPORARY TABLE CONSOLIDATED_MATERIAL_PLANT_STG_SAPC11 AS
	...
	select 'C11' as  SOURCE_SYS,
		...
		from ( 
			...
			From  sapc11.marc_current MARC
			join  sapc11.makt_current MAKT
			...
			on(trim (leading '0' from MARC.MATNR) = uc.u_item AND MARC.WERKS = uc.u_loc)  --where MARC.loaddts>v_last_extract_timestamp
	...
	using CONSOLIDATED_MATERIAL_PLANT_STG_SAPC11 material_plant_stg
	...
	select * from CONSOLIDATED_MATERIAL_PLANT_STG_SAPC11;
