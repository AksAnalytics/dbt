CREATE OR REPLACE PROCEDURE edw_consolidated.consolidated_deliveries_sape03_sp()
	LANGUAGE plpgsql
AS $$
...
begin
    ...
    drop table if exists CONSOLIDATED_DELIVERIES_STG_SAPE03;
   
	CREATE TEMPORARY table CONSOLIDATED_DELIVERIES_STG_SAPE03 as
			WITH LIKP AS ( select
	LIKP.VBELN ,
	LIKP.LFART ,	
	LIKP.WERKS ,	
	...
	case when replace(LIKP.ZZADLYEDAT , '-', '') ~ '^[0-9\.]+$' then replace(LIKP.ZZADLYEDAT, '-', '')::bigint
    else 00000000::bigint
	end ZZADLYEDAT,
	
	LIKP.INCO1 , 
	LIKP.INCO2 from sape03.likp_current LIKP where LIKP.LFART <>  '7'
...
select 
	...
	using CONSOLIDATED_DELIVERIES_STG_SAPE03 deliveries_stg
	...
	select * from CONSOLIDATED_DELIVERIES_STG_SAPE03;
