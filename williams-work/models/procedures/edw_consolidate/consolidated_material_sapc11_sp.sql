CREATE OR REPLACE PROCEDURE edw_consolidated.consolidated_material_sapc11_sp()
	LANGUAGE plpgsql
AS $$
...
begin
    ...
    drop table if exists CONSOLIDATED_MATERIAL_STG_SAPC11;
   
	CREATE TEMPORARY TABLE CONSOLIDATED_MATERIAL_STG_SAPC11 AS
	with txt_mat_grp as (
	...
	from edw.sapc11.mara_current MARA
	join edw.sapc11.makt_current MAKT
	on (MARA.MATNR = MAKT.MATNR AND MAKT.SPRAS='E' )
	left join edw.sapc11.zgppsbut_current ZGPP_SBUT
	on ( ZGPP_SBUT.SPRAS = 'E' and ZGPP_SBUT.zsbu = SUBSTRING(MARA.WRKST,1,3))
	left join edw.sapc11.zgppdivt_current ZGPP_DIVT
	on ( ZGPP_DIVT.SPRAS = 'E' and ZGPP_DIVT.zspart = SUBSTRING(MARA.WRKST,5,2))
	left join edw.sapc11.zgppcatt_current ZGPP_CATT
	on ( ZGPP_CATT.SPRAS = 'E' and ZGPP_CATT.zcategory = SUBSTRING(MARA.WRKST,8,3))
	left join edw.sapc11.zgppport_current ZGPP_PORTT
	...
	using CONSOLIDATED_MATERIAL_STG_SAPC11 material_stg
	...
	select * from CONSOLIDATED_MATERIAL_STG_SAPC11;
